...
- [ ] **Step 2: Activity Feed UI**
  - **Files:** `components/views/ActivityLog.tsx`, `components/ItemPanel.tsx`
  - **Logic:** Create a component that fetches `audit_logs` via TanStack Query. Embed this component inside `ItemPanel.tsx` (or as a board-level tab) wrapped in a role check so only admins see it.
  - **Verification:** Sign in as an Admin, make an edit, and verify the log appears in the UI.
