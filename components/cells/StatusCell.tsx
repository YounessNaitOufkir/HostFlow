  ...
  return (
    <div className={`${column.width ? '' : 'w-32'} border-r border-gray-200 dark:border-slate-700 p-[2px] relative flex items-center justify-center shrink-0 ${activeStatusId === cellKey ? "z-50" : ""}`} style={{ width: column.width ? `${column.width}px` : undefined }}>
      <div
        ...
        className={`cursor-pointer w-full h-full flex items-center justify-center text-white text-xs font-semibold rounded-[3px] hover:opacity-85 transition-opacity ${bgColor}`}
      >
        {value || ""}
      </div>

      {activeStatusId === cellKey && (
        <div className="absolute top-full mt-1 w-[140px] dropdown-menu p-1.5 z-50 flex flex-col space-y-0.5">
